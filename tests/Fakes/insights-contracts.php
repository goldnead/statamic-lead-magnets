 ...
 * ── The third file ────────────────────────────────────────────────────────
 *
 * `TableMetric` used to be in here, and that was the weak spot. The metrics in
 * `src/Integrations/Insights` **extend** it, and it is a class with behaviour
 * rather than a signature: it windows the period, truncates a timestamp in
 * three SQL dialects, splits a column without dropping the null rows and
 * narrows by brand. A reflection comparison cannot see a method **body**, so a
 * repair made upstream — a missing `whereNotNull`, an inclusive upper bound
 * that a binding truncates to the second — left this suite green over a copy
 * that no longer matched, and the metrics that had inlined the same body wrong
 * with it.
 *
 * So `insights-table-metric.php` beside this file is a **byte-for-byte copy** of
 * `statamic-insights/src/Support/TableMetric.php` and nothing else: no guard, no
 * header, not one character. The guard is at the call site instead
 * (`class_exists` before the `require_once`), and
 * `InsightsContractsMatchTest::the_copied_table_metric_is_byte_for_byte_the_real_one`
 * compares the two files' contents.
 *
