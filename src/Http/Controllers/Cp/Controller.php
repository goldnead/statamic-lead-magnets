...
abstract class Controller extends BaseController
{
     ...
     * short-circuits super users, so `can()` is correct for BOTH the file and
     * the eloquent users repository. Calling `hasPermission()`, `isSuper()` or
     * `id()` on the raw auth user crashes on eloquent-driver sites where the
     * authenticated model is a plain `App\Models\User`.
     *
     * Hiding a button in the template is not authorization. Every write route
     * in this addon calls this, and `tests/Feature/CpWriteRouteAuthorizationTest.php`
     * walks the route list to prove none was forgotten.
     */
